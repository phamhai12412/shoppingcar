...
function ShoppingCart() {
  ...
  const [keyadd, setkey] = useState(null);
  const settt = (set, key) => {
    settrangthaimua(set);
    setkey(key);
    //nếu có trong giỏ thì thôi chưa thì pus

    setgiohang([...giohang, { id: key, datagiohang: dataitem[key] }]);
  };

  const [themsl, setthemsl] = useState(0);
  const thembot = (thembotit) => {
    if (thembotit < 0) {
      setthemsl(0);
    } else setthemsl(thembotit);
  };
  ...
  return (
    <div className="container">
      <div className="giohang">️🛒</div>

      <Listitem
        dataitem={dataitem}
        settrangthaimua={settt}
